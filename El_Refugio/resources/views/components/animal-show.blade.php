        ...
        @if ($gallery->isNotEmpty())
            <div class="animal-detail-card__image-frame">
                ...
                @if ($gallery->count() > 1)
                    <button
                        type="button"
                        class="animal-detail-card__nav animal-detail-card__nav--prev"
                    ...
                    <button
                        type="button"
                        class="animal-detail-card__nav animal-detail-card__nav--next"
            ...
            @if ($gallery->count() > 1)
                <div class="animal-detail-card__thumbnails">
                    @foreach ($gallery as $index => $image)
                        <button
                            type="button"
                            class="animal-detail-card__thumb-btn {{ $index === 0 ? 'is-active' : '' }}"
                            data-index="{{ $index }}"
                        ...
                        >
                            <img
                                src="{{ $image['url'] }}"
                                alt="{{ $image['alt'] }}"
                                class="animal-detail-card__thumb"
                            >
                        </button>
                    @endforeach
                </div>
    ...
    <div class="animal-detail-card__info">
        ...
        @if ($animal->availability === AnimalAvailability::AVAILABLE)
            <div class="section-cta animal-detail__cta">
                <a href="{{ route('public.forms.request', $animal->id) }}" class="btn-card">
